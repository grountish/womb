<template>
    <div :class="[returnThemeClass(true, 'orange'), 'min-h-screen p-4']">
        <h1 :class="['heading-2 py-20',returnThemeClass(false, 'green')]">Our partners are lorem ipsum dolor sit amet</h1>
        <div class="">
            <div class="grid grid-cols-2 lg:grid-cols-3">
                <div v-for="(element, index) in collection" :key="index" class="-my-1">
                    <h2 :class="[index === currentIndex ? 'text-white': returnThemeClass(false, 'blue'), 'text-2xl font-black cursor-pointer min-w-18']" @click="currentIndex = index">
                        {{ element.name }}
                    </h2>
                </div>
            </div>
