<template>
	<article :class="['p-2 md:p-4 pt-24 md:pt-48', returnThemeClass(true, 'purple')]">
		...
		<section class="grid grid-cols-2 gap-x-2 gap-y-4 md:gap-y-2 md:grid-cols-3">
			<NuxtLink :to="'projects/' + project.slug.current" v-for="project in projects" :key="project._id">
		...
		<h1 :class="['heading-2 text-center py-24', returnThemeClass(false, 'yellow')]">
			{{ section4.blocks.subHeader }}
		</h1>
	</article>
</template>
...
export default {
	...
	async asyncData({ $sanity }) {
		...
		const { projects, section4 } = data
		return { projects, section4 }
	},
}
